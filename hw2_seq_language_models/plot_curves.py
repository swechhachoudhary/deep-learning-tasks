...
for exp in range(13, 19):
    ...
    # plt.ylim(top=250)
    plt.legend()
    plt.title("Exp " + str(exp) +
              ": Learning curves of PPL over wall-clock-time")
...
# plt.ylim(bottom=60)
plt.legend()
plt.title("ADAM: validation PPL over epochs for all experiments")
...
# plt.ylim(bottom=60)
plt.legend()
plt.title("ADAM: Validation PPL over wall-clock-time for all experiments")
