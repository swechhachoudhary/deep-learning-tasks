...
model.load_state_dict(torch.load('best_models/best_params_RNN.pt', map_location=lambda storage, loc: storage))
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
        if args.model == 'TRANSFORMER':
            ...
            targets = torch.from_numpy(y.astype(np.int64)).transpose(
                0, 1).contiguous().to(device)  # size is (seq_len, batch_size)
            print(targets.shape)
            print(inputs.shape)
        ...
        loss = loss_fn(outputs, targets)  # (seq_len, batch_size)
        loss_t = loss.sum(1) / model.batch_size  # loss.data.item() * model.seq_len

    return loss_t.detach().numpy()
...
model.batch_size = len(valid_data) // (model.seq_len + 1)
...
plt.legend()
plt.title('The average loss at each time-step for RNN')
plt.savefig('plots/loss_vs_t_rnn.png')
plt.close()
